...
<form method="post" action="actions/signup_user.php" class="mt-20 mx-auto max-w-lg p-4 flex flex-col min-w-md bg-base-100 rounded-lg shadow-lg">
    ...
    <div class="mt-10 relative">
        ...
        <input type="text" name="username" pattern="[a-zA-Z0-9\s]*" class="input w-full h-14 pt-3" required />
    </div>
    ...
    <div class="mt-4 relative">
        ...
        <input type="password" name="password" pattern="[a-zA-Z0-9\s]*" class="input w-full h-14 pt-3" required/>
    </div>
    <div class="mt-4 relative">
        <p class="absolute text-sm z-10 translate-x-3 translate-y-1 opacity-50">비밀번호 확인</p>
        <input type="password" name="password_repeat" pattern="[a-zA-Z0-9\s]*" class="input w-full h-14 pt-3" required/>
    </div>
